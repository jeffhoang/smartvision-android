...
class RtmpStreamTransport(
    ...
    // Buffer SPS/PPS in case codec config arrives before connection succeeds.
    ...
    override suspend fun startStreaming(preset: StreamPreset) {
        ...
        val url = preset.makePublishUrl().getOrElse {
            eventDidChange?.invoke(StreamingTransportEvent.Disconnected(it.message ?: "Invalid RTMP URL"))
            return
        }
        ...
        val client = buildRtmpClient(scope)
        ...
        diagnosticsStore.log("RTMP connecting: ${redactedUrl(url)}", DiagnosticsEntry.Category.stream)
    ...
    private fun buildRtmpClient(scope: CoroutineScope): RtmpClient =
        RtmpClient(object : ConnectChecker {
            override fun onConnectionStarted(url: String) {}

            override fun onConnectionSuccess() {
                reconnectAttempt = 0
                diagnosticsStore.log("RTMP connected", DiagnosticsEntry.Category.stream)
                val sps = pendingSps
                val pps = pendingPps
                if (sps != null && pps != null) {
                    rtmpClient?.setVideoInfo(ByteBuffer.wrap(sps.copyOf()), ByteBuffer.wrap(pps.copyOf()), null)
                }
                eventDidChange?.invoke(StreamingTransportEvent.Connected)
            }

            override fun onConnectionFailed(reason: String) {
                diagnosticsStore.log("RTMP connection failed: $reason", DiagnosticsEntry.Category.stream)
            ...
            override fun onDisconnect() {
                diagnosticsStore.log("RTMP disconnected", DiagnosticsEntry.Category.stream)
            }

            override fun onAuthError() {
                diagnosticsStore.log("RTMP auth error", DiagnosticsEntry.Category.error)
                eventDidChange?.invoke(StreamingTransportEvent.Disconnected("RTMP authentication failed"))
            }
    ...
    private suspend fun handleConnectionFailed(reason: String) {
        ...
        diagnosticsStore.log(
            "RTMP reconnect $reconnectAttempt/$maxAttempts in ${delaySecs}s",
            DiagnosticsEntry.Category.stream,
        )
        delay(delaySecs * 1_000L)
        val url = activePreset?.makePublishUrl()?.getOrNull() ?: return
        val client = rtmpClient ?: return
        // Re-send codec info before reconnecting.
        val sps = pendingSps
        val pps = pendingPps
        if (sps != null && pps != null) {
            client.setVideoInfo(ByteBuffer.wrap(sps.copyOf()), ByteBuffer.wrap(pps.copyOf()), null)
        }
        withContext(Dispatchers.IO) { client.connect(url, isRetry = true) }
    }
    ...
    private fun redactedUrl(url: String): String {
        val parts = url.split("/")
        return if (parts.size > 3) "${parts.dropLast(1).joinToString("/")}/**" else "rtmp://***"
    }
}
